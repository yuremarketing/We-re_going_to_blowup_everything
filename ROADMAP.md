...
- [x] VFX de impacto e feedback "juicy" (hit flash, partículas, screen shake) — **concluído**: implementados Hit Flash via duplicação de `StandardMaterial3D` e `Tween` nos inimigos e chefão, Screen Shake procedural no `Camera3D` do jogador com reset explícito de offset, e `CPUParticles3D` leves para morte/impacto. Testado e validado em modo headless no Godot 4.3 stable. — [issue #3](https://github.com/yuremarketing/We-re_going_to_blowup_everything/issues/3)
